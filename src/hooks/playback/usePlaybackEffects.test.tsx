...
describe('usePlaybackEffects', () => {
  ...
  it('bounds failed transition attempts and retries after returning to the foreground', async () => {
    const activePlayback = buildActivePlayback()
    const audio = document.createElement('audio')
    const play = vi.fn().mockRejectedValue(new Error('background playback suspended'))
    Object.defineProperty(audio, 'play', { configurable: true, value: play })
