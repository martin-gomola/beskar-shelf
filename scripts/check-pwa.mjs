...
const runtimeLifecycleSource = read('packages/pwa-runtime/src/index.ts')
...
requireCondition(sourceWorker.includes("'/index.html'"), 'service worker must precache index.html')
requireCondition(sourceWorker.includes("caches.match('/index.html')"), 'offline navigation must fall back to index.html')
...
const lifecycleImplementation = `${lifecycleSource}\n${runtimeLifecycleSource}`
requireCondition(lifecycleImplementation.includes("addEventListener('updatefound'"), 'lifecycle must observe installing workers')
requireCondition(lifecycleImplementation.includes("addEventListener('controllerchange'"), 'lifecycle must observe activation')
requireCondition(lifecycleImplementation.includes("updateViaCache: 'none'"), 'worker registration must bypass HTTP cache')
requireCondition(lifecycleImplementation.includes('60 * 60 * 1000'), 'automatic checks must be hourly')
requireCondition(lifecycleImplementation.includes("document.visibilityState === 'visible'"), 'visible apps must check overdue updates')
