...
| `ABS_UPSTREAM` | Internal Audiobookshelf origin the app container proxies to |
...
Point `ABS_UPSTREAM` at a private/internal Audiobookshelf origin that the app
container can reach directly, such as `http://host.docker.internal:13378` on
Docker Desktop or an internal service name like `http://audiobookshelf:80`.
Avoid pointing it at the same public hostname fronted by Cloudflare or another
reverse proxy, because the extra hop can cause upstream connection failures and
break playback/ebook loads.
